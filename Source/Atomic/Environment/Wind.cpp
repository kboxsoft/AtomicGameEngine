...
#include <Atomic/Environment/Attribute.h>
...
{
    context->RegisterFactory<WindSystem>(GEOMETRY_CATEGORY);

    ATOMIC_COPY_BASE_ATTRIBUTES(LogicComponent);
    ATOMIC_ACCESSOR_ATTRIBUTE("Pulse Frequency", GetPulseFreqency, SetPulseFrequency, float, 0.0f, AM_DEFAULT | AM_NOEDIT);
}
...
{
    context->RegisterFactory<WindZone>(GEOMETRY_CATEGORY);

    ATOMIC_COPY_BASE_ATTRIBUTES(Component);
    ATOMIC_ENUM_ACCESSOR_ATTRIBUTE("Type", GetWindZoneType, SetWindZoneType, WindZoneType, windZoneTypesNames, 0, AM_DEFAULT);
    ATOMIC_MEMBER_ATTRIBUTE("Radius", float, radius_, 0.0f, AM_DEFAULT);
    ATOMIC_MEMBER_ATTRIBUTE("Main", float, main_, 0.0f, AM_DEFAULT);
    ATOMIC_MEMBER_ATTRIBUTE("Turbulence", float, turbulence_, 0.0f, AM_DEFAULT);
    ATOMIC_MEMBER_ATTRIBUTE("Pulse Magnitude", float, pulseMagnitude_, 0.0f, AM_DEFAULT);
    ATOMIC_ACCESSOR_ATTRIBUTE("Pulse Frequency", GetPulseFreqency, SetPulseFrequency, float, 0.0f, AM_EDIT);
}
